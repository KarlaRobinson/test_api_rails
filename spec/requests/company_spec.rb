...
RSpec.describe 'Payments API', type: :request do
  ...
  describe 'POST /companies' do
    # valid payload
    let(:valid_attributes) { { name: 'Rappi', category: 'delivery services' } }

    context 'when the request is valid' do
      before { post '/companies', params: valid_attributes }

      it 'creates a company' do
        expect(JSON.parse(response.body)['name']).to eq('Rappi')
        expect(JSON.parse(response.body)['activated']).to eq(false)
      end
  ...
  describe 'PUT /companies/:id' do
    let(:valid_attributes) { { name: 'Uber' } }
